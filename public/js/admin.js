...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function fetchOrders() {
    try {
      const response = await adminFetch(`${API_BASE}/api/orders`);
      if (!response.ok) {
...
function formatDate(date) {
  ...
  function renderOrders(orders) {
    ...
    document.querySelectorAll('.btn-status').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        try {
          const response = await adminFetch(`${API_BASE}/api/orders/${id}`, {
            method: 'PUT',
  ...
  function connectSSE() {
    const token = localStorage.getItem('fermaja_admin_token') || '';
    const eventSource = new EventSource(`${API_BASE}/api/events?token=${encodeURIComponent(token)}`);
